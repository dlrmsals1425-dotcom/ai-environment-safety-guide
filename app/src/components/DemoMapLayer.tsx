...
export function DemoMapLayer({map}:{map:MapLibreMap}) {
  ...
  useEffect(()=>{
    ...
    const apply=()=>{
      ...
      try{
        ...
        (map.getSource(SOURCE) as GeoJSONSource).setData({type:'FeatureCollection',features:enabled?features:[]});
  ...
  useEffect(()=>{
    ...
    const markers=results.slice(0,12).map(r=>{
      ...
      element.textContent=`가상 ${r.name.slice(-1)} · ${r.priority}점`;
  ...
  },[map,enabled,results,selected,plan]);
  ...
  return enabled?<div className="demo-map-legend"><strong>가상 시나리오 · 실제 위험 아님</strong><span>분홍: 제설 우선 검토 · 주황: 결빙 우려 확인</span><span>보라: 모니터링 · 청색 테두리: 계획에 담음</span><span>180m 시연 사각형 · 실제 도로·위험 경계 아님</span>{error&&<span role="alert">{error}</span>}</div>:null;
}
